...
import static com.shouyubang.android.sybang.record.CommonUtils.SIZE_1;
import static com.shouyubang.android.sybang.record.CommonUtils.SIZE_2;
import static com.shouyubang.android.sybang.record.CommonUtils.getScreenHeight;
import static com.shouyubang.android.sybang.record.CommonUtils.getScreenWidth;
...
public class RecordActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        mLayoutRoot = (CoordinatorLayout) findViewById(R.id.activity_record);
    ...
    private void startRecordUI() {
        ...
        recordButton.setImageResource(R.mipmap.stop_record);   //录制按钮变成待停止
    }
    ...
    private void endRecordUI() {
        ...
        recordButton.setImageResource(R.mipmap.record);   //录制按钮变成待停止
    }
    ...
    private void FlashLogic(Camera.Parameters p, int type, boolean isOn) {
        ...
        if (type == 0) {
            ...
            videoFlashLight.setImageResource(R.drawable.ic_flash_off_white_24dp);
        } else {
            if (isOn) {
                p.setFlashMode(Camera.Parameters.FLASH_MODE_TORCH);
                camera.setParameters(p);
            }
            videoFlashLight.setImageResource(R.drawable.ic_flash_on_white_24dp);
        }
        if (cameraFlag == 0) {
    ...
    private void askPermissions() {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.M) {
            if ((ActivityCompat.checkSelfPermission(RecordActivity.this, Manifest.permission.CAMERA)
            ...
            Snackbar.make(mLayoutRoot, R.string.permissions_not_granted, Snackbar.LENGTH_SHORT).show();
        }
    }
    ...
    public String videoDir() {
        ...
        // 创建文件
        try {
            videoFile = File.createTempFile("recording", ".mp4", recordDir);
        } catch (IOException e) {
            e.printStackTrace();
        }
