...
import com.shouyubang.android.sybang.presenters.view.LogoutView;
import com.shouyubang.android.sybang.presenters.view.UploadVideoView;
import com.shouyubang.android.sybang.record.BizService;
import com.shouyubang.android.sybang.record.PutObjectTask;
...
public class AccountHelper {
    ...
    private class StandardLoginTask extends AsyncTask<String, Integer, RequestBackInfo> {
        ...
        protected RequestBackInfo doInBackground(String... strings) {
            ...
            return UserServerHelper.getInstance().loginId(id, password);
        }
    ...
    public void standardRegister(final String id, final String psw) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                final RequestBackInfo result = UserServerHelper.getInstance().registerId(id, psw);
                if (null != mContext) {
    ...
    public void standardLogout(final String id) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                RequestBackInfo result = UserServerHelper.getInstance().logoutId(id);
     ...
     * 上传
     */
    public void upload(final BizService bizService, final String currentPath) {
        new Thread(new Runnable() {
            @Override
            public void run() {
                String filename = com.tencent.cos.utils.FileUtils.getFileName(currentPath);
                String cosPath = "/" + filename; //cos 上的路径
                PutObjectTask.putObjectForLargeFile(AccountHelper.this, bizService, cosPath, currentPath);
            }
        }).start();
    }
}
